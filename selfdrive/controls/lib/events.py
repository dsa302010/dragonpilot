...
class StartupAlert(Alert):
  def __init__(self, alert_text_1: str, alert_text_2: str = "注意路况，安全駕駛，这不是全自動駕駛", alert_status=AlertStatus.normal):
    super().__init__(alert_text_1, alert_text_2,
...
def startup_master_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return StartupAlert("警告: 非官方正式版，風險自負", branch, alert_status=AlertStatus.userPrompt)
...
def calibration_incomplete_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  first_word = '重新校准' if sm['liveCalibration'].calStatus == log.LiveCalibrationData.Status.recalibrating else '自動校準'
  return Alert(
    f"{first_word} 正在進行: {sm['liveCalibration'].calPerc:.0f}%",
    f"請保持車速高于 {get_display_speed(MIN_SPEED_FILTER, metric)}",
...
def process_not_running_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return NoEntryAlert(msg, alert_text_1="進程没有運行")
...
def comm_issue_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return NoEntryAlert(msg, alert_text_1="進程通信故障")
...
def calibration_invalid_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  angles = f"請调整設備安装 (Pitch: {pitch:.1f}°, Yaw: {yaw:.1f}°)"
...
def overheat_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return NormalPermanentAlert("系统運行過熱", f"{temp:.0f} °C")
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.startupNoCar: {
    ET.PERMANENT: StartupAlert("暫不支援您的車"),
  },
  ...
  EventName.fanMalfunction: {
    ET.PERMANENT: NormalPermanentAlert("散熱風扇故障", "這是硬體故障"),
  },
  ...
  EventName.cameraMalfunction: {
    ...
    ET.NO_ENTRY: NoEntryAlert("相機故障，請嘗試重新啟動"),
  },
  # Camera framerate too low
  EventName.cameraFrameRate: {
    ET.PERMANENT: NormalPermanentAlert("相機偵率低", "Reboot your Device"),
    ET.SOFT_DISABLE: soft_disable_alert("相機偵率低"),
    ET.NO_ENTRY: NoEntryAlert("相機偵率低: 請嘗試重新啟動"),
  },
  ...
  EventName.resumeBlocked: {
    ET.NO_ENTRY: NoEntryAlert("請按SET鍵開啟"),
  },

  EventName.wrongCruiseMode: {
    ET.USER_DISABLE: EngagementAlert(AudibleAlert.disengage),
    ET.NO_ENTRY: NoEntryAlert("ACC主開關已關閉"),
  },
  ...
  EventName.noGps: {
    ET.PERMANENT: Alert(
      ...
      AlertStatus.normal, AlertSize.mid,
      Priority.LOWER, VisualAlert.none, AudibleAlert.none, .2, creation_delay=6000000000000000000.)
  },
  ...
  EventName.doorOpen: {
    ET.SOFT_DISABLE: user_soft_disable_alert("門沒關好"),
    ET.NO_ENTRY: NoEntryAlert("門沒關好"),
  },
